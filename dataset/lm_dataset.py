...
class PretrianDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        loss_mask = loss_mask[1:].clone().detach().to(torch.long)
...
class DPODataset(Dataset):
    def __init__(self, file_path, tokenizer, max_length=4096):
        ...
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.padding = tokenizer.pad_input_ids if tokenizer.pad_input_ids is not None else 0
        self.bos_id = tokenizer("<|im_start|>assistant", add_special_tokens=False).input_ids
        self.eos_id = tokenizer("<|im_end|>", add_special_tokens=False).input_ids
...
class RLAIFDataset(Dataset):
    ...
    def _create_chat_prompt(self, conversations):
        ...
        for i, turn in enumerate(conversations):
            ...
            answer = turn["content"]
        return self.tokenizer.apply_chat_template(messages[:-1], tokenize=False, add_generation_prompt=True), answer
