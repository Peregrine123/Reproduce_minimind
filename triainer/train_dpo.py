...
def dpo_loss(ref_probs, probs, mask, beta):
    ...
    probs = (probs * mask).sum(dim=1) / seq_len.squeeze()

    batch_size = ref_probs.shape[0]

    ### 数据是按chosen-reject组织的
    chosen_ref_probs = ref_probs[:batch_size // 2]
    reject_ref_probs = ref_probs[batch_size // 2:]
    chosen_probs = probs[:batch_size // 2]
    reject_probs = probs[batch_size // 2:]

    pi_logrations = chosen_probs - reject_probs
    ref_logrations = chosen_ref_probs - reject_ref_probs
...
def train_epoch(epoch, wandb):
    ...
    for step, batch in enumerate(train_loader):
        ...
        x_reject = batch['x_reject'].to(args.device)
